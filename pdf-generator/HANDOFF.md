...
- `.eligbar` — **עיצוב הערת הזכאות שנבחר**: פס ענבר בצד, **בלי מסגרת**, עם אוויר נדיב מעל/מתחת (margin 26/30). עוזר: `elig_note()`. כרגע על **02 בלבד**.
- `.notice` — הגרסה הישנה של הערת הזכאות (מסגרת "שים לב" אדומה דקה). עדיין על 03/04 עד שמחילים את `.eligbar`.
...
## החלטות אחרונות (לא לשנות בלי אישור)
- **מספור עמודים בדסקטופ — להוסיף החל מ-05.** ב-`render.js`, בענף ה-desktop (paginate) להוסיף
  `displayHeaderFooter:true` + `footerTemplate` עם מספר עמוד (ממורכז, פונט קטן, צבע אפור). **מובייל = עמוד אחד, בלי מספור.**
  צריך גם להגדיל `margin.bottom` בדסקטופ כדי שהמספר לא יתנגש בפוטר. בסוף להחיל גם על 01–04 לאחידות.
...
- להחיל את עיצוב `.eligbar` + הניסוח החדש גם על 03/04 (כרגע רק 02).
- להמשיך 05–08 (הבא: 05 הוצאות נסיעה, אייקון Bus) — **עם מספור עמודים בדסקטופ** (ראו "החלטות אחרונות").
