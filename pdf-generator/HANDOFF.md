...
## סט המסירה (delivery/) — מבנה חדש, מסירה מדורגת
כדי להתאים את המסירה לתהליך, נבנה **סט מסירה מעודכן** בתיקייה `delivery/` (דסקטופ + מובייל), **בלי לדרוס** את `pdf/` הקפוא או את ה-builders המקוריים. מבנה הסט וסדר המסירה:

| מסמך | מתי נמסר לפונה |
|---|---|
| **00-benefits-map** — מפת הזכאויות (חדש) | בהתחלה, יחד עם 01a — התמונה המלאה מראש, כל זכאות עם תנאי-הסף שלה (מסגור מותנה, לא הבטחה) |
| **01a-application** — הגשת בקשה לאישור לימודים | בהתחלה. = 01 המקורי **עד וכולל** "איך אדע אם הבקשה שלי אושרה?" |
| **01b-schedule-and-benefits** — הגשת מערכת שעות וקביעת הזכאויות | אחרי אישור הלימודים, **וחוזר בכל תחילת סמסטר**. = חלק "מערכת שעות" + "מימוש" מ-01, עם Hero משלו |
| **02–08** — הזכאויות | אחרי שהזכאויות נקבעו ("אישור לימודים לסמסטר"), מאוצר לפי הרלוונטי |

**איך בונים את הסט:** `./build-delivery.sh` → `delivery/desktop/*.pdf` + `delivery/mobile/*.pdf`.

**וריאנט סגנון "לבן נקי" (אפשרות 2):** בנוסף לסט הצבעוני, קיים וריאנט לבן ל-01a ו-01b בלבד (הם היחידים שמשתמשים באזורי-רקע צבעוניים לפי שלב). בלבן אין אזור-רקע נמתח — הצבע עובר לסמני-השלבים, למסגרות הכרטיסים ולאייקונים; זה מבטל את השטח הצבעוני הריק שנשאר בשבירת עמוד בדסקטופ. ה-01a הלבן גם מוסיף כותרת-מעבר ירוקה "קבלת ההחלטה / לאחר בחינת הבקשה" לפני כרטיס "איך אדע אם הבקשה שלי אושרה?". בנייה: `./build-delivery-white.sh` → `delivery/white/desktop|mobile/`. הבנאים: `pages/app01a_white.py`, `app01b_white.py` (עותקים עצמאיים; הצבעוניים המקוריים לא נגעים). 00 ו-02–08 חסרי אזורים ולכן זהים בשני הסגנונות — לא משוכפלים ב-`white/`.

**המנגנון (אפס דריסה):** `build_delivery.py` בונה 00/01a/01b מ-builders חדשים (`pages/app00.py`, `app01a.py`, `app01b.py` — עצמאיים כמו שאר העמודים). ל-02–08 הוא **מייבא את ה-builder המקורי ודורס שתי פונקציות בזמן ריצה** (build() מאתר אותן כגלובלים בזמן קריאה):
- `elig_note()` → ריק (מוריד את פס-הענבר העליון).
- `important()` → תיבת "חשוב לזכור" עם **מסגור-הציפיות** (שהועבר מהפס העליון). תזכורת הגשת-הסמסטר הישנה עברה ל-01b.

כך ראש עמוד הזכאות נקי ("בקצרה" → ישר לתוכן), והסיום עקבי — בלי לגעת ב-`app02–08.py`, `docs/`, או `pdf/`.
