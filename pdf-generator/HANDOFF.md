...
## סטטוס
- ✅ 00 (מימוש זכאויות — סל התמיכות) — **עמוד אינדקס**: Hero (Sparkles) + כרטיס פתיח + גריד 7 כרטיסים צבעוניים (צבע לכל זכאות, נאמן למקור) + הערת סיום. עמוד דסקטופ אחד. **הקישורים בכרטיסים = placeholder — להציב בהמשך.**
...
- `.bgrid`/`.bcard`(+צבעים `.c-amber`/`.c-blue`/`.c-emerald`/`.c-sky`/`.c-violet`/`.c-rose`/`.c-teal`)/`.bnote`/`.hero-badge` — **עמוד האינדקס (00)** בלבד. הכרטיסים ב-`app00.py` (מערך `BENEFITS`); כדי להוסיף קישור — למלא `href` בכל שורה (כרגע `None`).
...
## פתוח / TODO
- **קישורים בעמוד האינדקס (00):** הכרטיסים כרגע placeholder. להציב `href` לכל זכאות במערך `BENEFITS` שב-`app00.py`.
- פוטר/"חשוב לזכור" לעיתים נדחפים לעמוד אחרון כמעט-ריק (מינורי; קורה גם ב-06/08).
- **כל העמודים (00–08) הושלמו** — נשאר ליטוש/הגהה לפי משוב.
