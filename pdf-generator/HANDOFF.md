...
| מסמך | מתי נמסר לפונה |
|---|---|
| **benefits-hub** — מימוש זכאויות (עמוד ניווט, בסגנון /benefits באתר) | אחרי שהזכאויות נקבעו — hub ניווט: 7 כרטיסי זכאות, כל אחד **קישור לחיץ** לקובץ הזכאות. יעדי הקישורים ב-`pages/app_hub.py` (`LINKS`) — למלא בכתובות Google Drive. (החליף את "מפת הזכאויות" הישנה.) |
...
**סגנון "לבן נקי" (הסגנון הראשי):** 01a ו-01b **אינם** משתמשים יותר באזורי-רקע צבעוניים לפי שלב. אין אזור-רקע נמתח — הצבע עובר לסמני-השלבים, למסגרות הכרטיסים ולאייקונים; זה מבטל את השטח הצבעוני הריק שנשאר בשבירת עמוד בדסקטופ (`part()` ב-app01a/app01b מחזיר רקע שקוף). כותרות-מעבר (עוזר `phase("green","chevD",…)`) הוספו: ב-01a "קבלת ההחלטה / לאחר בחינת הבקשה" לפני "איך אדע אם הבקשה שלי אושרה?"; ב-02 "איך נחשב את גובה דמי השיקום שלך" ו-"תשלום דמי השיקום" לפני הסקשנים המתאימים.
