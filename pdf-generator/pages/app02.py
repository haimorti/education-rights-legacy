...
def build():
    ...
    B.append(sechead("shield","תנאי הזכאות לדמי שיקום"))
    B.append(p("קבלת דמי השיקום מותנית בעמידה בשלושה תנאים עיקריים שנקבעו באגף השיקום.","lead"))
    B.append(cond("1","שיעור הקצבה לה הנך זכאי",
        p("דמי השיקום משלימים ל-100% את הקצבה המשולמת לך.")
        + callout("amber","אתה זכאי לקצבת נכות מלאה ולכן לא ישולמו לך דמי שיקום.","info")))
    B.append(cond("2","עומס לימודי",
    ...
    B.append(cond("3","הכנסות",
        p("בבדיקת הזכאות לתשלום דמי שיקום נלקחות בחשבון קצבאות מחליפות שכר (אבטלה, מילואים, דמי לידה) והכנסות שלא מעבודה.")))

    # ===== Section 2: hours calculation =====
    B.append(sechead("clock","איך מחושב היקף שעות הלימוד שלך?"))
    B.append(p("במסלולי לימודים אקדמיים, היקף שעות הלימוד שלך מחושב לפי "+b("הערך הגבוה")+" מבין השניים:","lead"))
    B.append(twoup([("calc","סך השעות השבועיות","סך השעות השבועיות המופיעות במערכת השעות שלך."),
                    ("cap","סך נקודות הזכות","סך נקודות הזכות של הקורסים הרשומים במערכת.")]))
    B.append(card(f'<div class="cond-h"><span class="sec-ico">{svg("info",17)}</span><h4>אוניברסיטה פתוחה</h4></div>'
        ...
        + checkbullets(["רשימת הקורסים שאליהם נרשמת באותו סמסטר","סך נקודות הזכות של הקורסים"])))

    # ===== Section 3: amount calculation =====
    B.append(sechead("calc","חישוב גובה דמי השיקום"))
    B.append(p("חישוב גובה דמי השיקום שונה בין נכה כללי לנפגע עבודה:","lead"))
    # --- נכה כללי ---
    B.append(card(dlabel("נכה כללי") + p("גובה דמי השיקום נקבע לפי שלושה פרמטרים מרכזיים:")))
    B.append(cond("1","דרגת אובדן הכושר שנקבעה לך",
    ...
    # --- נפגע עבודה ---
    B.append(card(dlabel("נפגע עבודה")
    ...
    B.append(sechead("wallet","מתי משולמים דמי השיקום?"))
    B.append(p("אם אתה עומד בתנאי הזכאות לדמי שיקום, עובד השיקום יוודא שהתשלום יועבר אליך באופן אוטומטי.","lead"))
    B.append(f'<div class="cond"><div class="cond-h"><span class="sec-ico">{svg("calcheck",18)}</span><h4>אחת לחודש</h4></div>'
             + p("על פי רוב עד לעשירי לחודש, עבור החודש הקודם.")+'</div>')
    B.append(callout("amber","לעיתים תשלום דמי השיקום מתעכב בשל עדכון גובה הכנסותיך מעבודה.","info"))
    B.append(callout("red",b("שים לב: ")+"הזכאות לדמי שיקום ניתנת רק בתקופות לימודים פעילות. לכן אין זכאות לתשלום דמי שיקום בחופשת הקיץ.","alert"))
